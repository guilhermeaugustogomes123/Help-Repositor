...
import { Product, Aisle, Palette, BoxItem, Alert, HistoryEntry, ReplenishmentItem, Priority, Promoter, Note, DayOff } from '../types'
...
const SEED_PROMOTERS: Promoter[] = [
  { id: 'pro1', name: 'Mariana Costa', company: 'Itambé', productIds: ['p1'], weekdays: ['seg', 'qua', 'sex'] },
  { id: 'pro2', name: 'Rafael Lima', company: 'Coca-Cola', productIds: ['p4', 'p5'], weekdays: ['ter', 'qui'] },
]

const SEED_NOTES: Note[] = [
  { id: 'note1', text: 'Conferir a ponta de gôndola do corredor 2', completed: false, createdAt: new Date().toISOString() },
]
...
const SEED_DAYS_OFF: DayOff[] = [
  { id: 'off1', date: futureDate(3), type: 'folga', notes: 'Folga semanal' },
  { id: 'off2', date: futureDate(18), type: 'ferias', notes: 'Início das férias' },
]
...
interface AppState {
  ...
  addPromoter: (p: Omit<Promoter, 'id'>) => void
  deletePromoter: (id: string) => void
  addNote: (text: string) => void
  toggleNote: (id: string) => void
  deleteNote: (id: string) => void
  addDayOff: (day: Omit<DayOff, 'id'>) => void
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [promoters, setPromoters] = useState<Promoter[]>(() => load('ss_promoters', SEED_PROMOTERS))
  const [notes, setNotes] = useState<Note[]>(() => load('ss_notes', SEED_NOTES))
  const [daysOff, setDaysOff] = useState<DayOff[]>(() => load('ss_days_off', SEED_DAYS_OFF))
  ...
  useEffect(() => { localStorage.setItem('ss_promoters', JSON.stringify(promoters)) }, [promoters])
  useEffect(() => { localStorage.setItem('ss_notes', JSON.stringify(notes)) }, [notes])
  useEffect(() => { localStorage.setItem('ss_days_off', JSON.stringify(daysOff)) }, [daysOff])
  useEffect(() => {
  ...
  function addPromoter(p: Omit<Promoter, 'id'>) {
    setPromoters(prev => [...prev, { ...p, id: uid() }])
  }

  function deletePromoter(id: string) {
    setPromoters(prev => prev.filter(p => p.id !== id))
  }

  function addNote(text: string) {
    setNotes(prev => [{ id: uid(), text, completed: false, createdAt: new Date().toISOString() }, ...prev])
  }
  ...
  function addDayOff(day: Omit<DayOff, 'id'>) {
    setDaysOff(prev => [...prev, { ...day, id: uid() }].sort((a, b) => a.date.localeCompare(b.date)))
  }

  function deleteDayOff(id: string) {
    setDaysOff(prev => prev.filter(day => day.id !== id))
  }

  return (
    <AppContext.Provider value={{
      products, aisles: AISLES, palettes, boxes, alerts, history, replenishment, promoters, notes, daysOff,
      ...
      addPromoter, deletePromoter, addNote, toggleNote, deleteNote, addDayOff, deleteDayOff,
    }}>
      {children}
    </AppContext.Provider>
  )
